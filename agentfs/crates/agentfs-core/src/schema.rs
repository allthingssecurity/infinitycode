...
/// DDL for schema v3 additions (memory metadata + FTS5 search).
const SCHEMA_V3_ADDITIONS: &str = r#"
-- Memory metadata for tiered storage and access tracking
...
CREATE INDEX IF NOT EXISTS idx_memory_tier ON memory_metadata(tier);
CREATE INDEX IF NOT EXISTS idx_memory_provider ON memory_metadata(provider);
CREATE INDEX IF NOT EXISTS idx_memory_accessed ON memory_metadata(last_accessed);
CREATE INDEX IF NOT EXISTS idx_memory_hash ON memory_metadata(content_hash);

-- Full-text search index for memory entries
...
mod tests {
    ...
    fn init_and_verify() {
        ...
        // v3 tables exist
        let memory_metadata_exists: bool = conn
            .query_row(
            ...
            .unwrap();
        assert!(memory_metadata_exists);

        let memory_fts_exists: bool = conn
            .query_row(
